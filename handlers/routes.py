...
def configure_routes(application, Session):
    ...
    def index():
        ...
        #     if os.path.isdir('static/dataset/' + session['username']):
        #         shutil.rmtree('static/dataset/' + session['username'])
        #         print(session['username'] + "was deleted!")
    ...
    def mobile_authentication(): 
        ...
        data = face_recognition_mobile(flask.request)
        return flask.jsonify(data)
    ...
    def redirector():
        if flask.request.method == "POST" and 'url' in request.form and 'name' in request.form and 'email' in request.form and 'token' in request.form and 'ide' in request.form:
            ...
            if ide > 0:
                ...
                user_str = session['useremail'] + ' ' + session['rfc'] + ' ' + str(request.form["phone"]) + '\n'
                if os.path.exists('user_information.txt'):
                    with open('user_information.txt', 'r') as f:
                        exist_str = str(f.read())
                    if exist_str.find(user_str) == -1:
                        user_str += exist_str
